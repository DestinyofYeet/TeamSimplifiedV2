...
public class TeamDemoteCommand {
    public boolean onCommand(CommandSender commandSender, Command command, String s, String[] args){
        ...
        if (rank != TeamRank.OWNER){
            player.sendMessage("§cSorry, only the owner can demote players!");
            return true;
        }
        ...
        if (targetPlayerRank == TeamRank.PLAYER){
            player.sendMessage("§cThat player already is a player!");
            return true;
        }

        team.promotePlayer(targetPlayer);

        player.sendMessage("§aSuccessfully demoted '§6" + targetPlayerName + "§a' to player!");

        targetPlayer.sendMessage("§aYou got demoted to player by '§6" + player.getName() + "§a'!");
